...
def summarize_events_with_claude(
    ...
    user_prompt = f"""以下のイベントデータをもとに、週次イベント情報メールの本文を日本語で作成してください。

【対象地域】オスロおよびその周辺（車で行ける範囲）、ヨーテボリ、クリスチャンサン
【期間】今後4週間以内

【フォーマット・ルール】
- 全体を400〜500文字以内に収める（簡潔さ最優先）
- カテゴリ別に分けず、日付順に一覧で並べる
- 1イベントあたり「📅日付 イベント名 📍会場/都市」の1行形式のみ
- 説明文・URLは不要
- 特に注目イベントには先頭に⭐をつける
- 冒頭に一言だけ（例：「今週のオスロ周辺イベントです。」）
- 末尾不要（署名・ソース一覧なし）
